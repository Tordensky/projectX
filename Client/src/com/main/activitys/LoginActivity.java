...
public class LoginActivity extends Activity {
	...
	private String registerUrl = "http://restserver.herokuapp.com/user/login";
	...
	public void postLoginInfo(View v){
		...
		if(username.getText().length() > 0){
			...
			try {
				...
				registerInfo.put("Password", Secure.getString(getBaseContext().getContentResolver(),
            Secure.ANDROID_ID));
	...
	public void confirmLogin(String responseBody){
		...
		try {
			...
			if(response.has("UID")){
				...
				Login.setUsername(loginSettings, response.getString("Username"));
